...
n = int(input())  # количество строк в тексте
words = {}

# считываем строки и подсчитываем частоту встречаемости слов
for _ in range(n):
    line = input().split()
    for word in line:
        words[word] = words.get(word, 0) + 1

# создаем список кортежей (частота, слово)
...
# сортируем список по убыванию частоты и лексикографическому порядку
word_freq.sort(key=lambda x: (-x[0], x[1]))

# выводим слова по одному на каждую строку
