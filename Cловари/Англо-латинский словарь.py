...
for _ in range(int(input())):
    eng_word,*ts = input().replace('-','').replace(',','').split(' ')
    ts.remove('')
    d[eng_word] = set(ts)
    for v in d.values(): #создаем список латинских слов
        a.update(v)
print(len(a))
for x in sorted(a): #перебираем латинские слова в отсортированном множестве a
    for k,v in d.items():
        if x in v:
            b.add(str(k)) #собираем все переводы латинского слова во множество b
    print(x,' - ',', '.join(sorted(set(b))))
    b = set()
